...
class WebSocketManager:
    ...
    def __init__(self):
        ...
        self.user_info: Dict[int, dict] = {}  # user_id -> {username, session_token}
    ...
    async def connect(self, websocket: WebSocket, user_id: int, session_token: str, username: str = ""):
        """处理新连接"""
        ...
        self.user_info[user_id] = {"username": username, "session_token": session_token}
    ...
    async def handle_join_table(self, user_id: int, data: dict):
        ...
        if not player:
            ...
            # directly insert into table.players dict (not via add_player since position may be auto)
            # We'll let add_player handle position assignment

        # 检查是否已在牌桌上
        if player.position is not None and player.is_active:
            await self.send_to_user(
                user_id,
                {
                    "type": "error",
        ...
        # 检查重新买入限制
        if buyin_type == "rebuy" and player.rebuy_count >= 1:
            await self.send_to_user(
                user_id,
                {
                    "type": "error",
                    "data": {"error": "每人限重新买入1次"}
                }
            )
            return
    ...
    def get_room_info(self) -> dict:
        """获取房间信息"""
        table_players = self.game_engine.table.players
        connected_count = len(self.user_to_connection)
